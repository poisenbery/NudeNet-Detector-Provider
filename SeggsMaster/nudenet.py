...
def read_image(image, target_size=320): #changes data format of comfyUI tensor
    image_pil = Image.fromarray(np.clip(255. * image.cpu().numpy().squeeze(0), 0, 255).astype(np.uint8))
    image_transposed = np.transpose(image_pil, (0, 1, 2))
    img_height, img_width = image_transposed.shape[:2] #new get h/w
    image_np = np.array(image_transposed)
    image_cv2 = image_np[:, :, ::-1]
    img = image_cv2.astype(np.uint8)
...
def postprocess(output, resize_factor, pad_left, pad_top):
...
class NudeDetector:
    ...
    def detect(self, image):
        ...
        detections = postprocess(outputs, resize_factor, pad_left, pad_top)
